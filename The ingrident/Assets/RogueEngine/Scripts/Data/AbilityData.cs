...
namespace RogueEngine
{
    ...
    public class AbilityData : ScriptableObject
    {
        ...
        //Only use for visual estimates during battles
        public int GetDamageEstimate(World world, Card card, BattleCharacter target)
        {
            BattleCharacter caster = world.battle.GetCharacter(card.owner_uid);

            int damage = 0;
            foreach (EffectData eff in effects)
            {
                if (eff != null && eff is EffectDamage)
                    damage += ((EffectDamage)eff).GetDamage(this, caster, card);
                if (eff != null && eff is EffectDamageHalf)
                    damage += ((EffectDamageHalf)eff).GetDamage(target);
                if (eff != null && eff is EffectDamageRandom)
                    damage += ((EffectDamageRandom)eff).GetDamage(world, this, caster, card);
            }
            return damage;
        }
        ...
        public List<BattleCharacter> GetCharacterTargets(Battle data, BattleCharacter caster, Card card, ListSwap<BattleCharacter> memory_array = null)
        {
            ...
            else if (target == AbilityTarget.AllCharacters)
            {
                foreach (BattleCharacter character in data.characters)
                {
                    if (!character.IsDead() && AreTargetConditionsMet(data, caster, card, character))
        ...
        public bool CanPlayTarget(Battle data, Card card, BattleCharacter character)
        {
            ...
            if (!AreTriggerConditionsMet(data, owner, card))
                return false;
    ...
    public enum AbilityTrigger
    {
        ...
        Ongoing = 2,  //Always active when in play (does not work with all effects)

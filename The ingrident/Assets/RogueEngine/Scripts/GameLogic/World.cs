...
namespace RogueEngine
{
    ...
    public class World
    {
        ...
        public bool CanControlChampion(int player_id, Champion champion)
        {
            if (champion == null)
                return false;
            Player cplayer = GetPlayer(champion.player_id);
            if (cplayer != null && !cplayer.connected && player_id >= 0)
                return true;
            return champion.player_id == player_id;
        }
        ...
        public virtual void SetActionCompleted(int player_id, bool completed)
        {
            foreach (Champion champ in champions)
            {
                if (CanControlChampion(player_id, champ))
                    champ.action_completed = completed;
            }
        }
        ...
        public Champion GetFirstChampion(int player_id)
        {
            //Find Self first
            foreach (Champion champion in champions)
            {
            ...
            //If not found, check that can be controlled
            foreach (Champion champion in champions)
            {
                if (CanControlChampion(player_id, champion))
                    return champion;
            }
            return null;
        }
        ...
        public Champion GetNextActionChampion(int player_id)
        {
            ...
            foreach (Champion champion in champions)
            {
                if (!champion.IsDead() && player_id == champion.player_id && !champion.action_completed)
                    return champion;
            }
            //Get other you control
            foreach (Champion champion in champions)
            {
                if (!champion.IsDead() && CanControlChampion(player_id, champion) && !champion.action_completed)
                    return champion;
            }
            return null;
        }
        ...
        public T GetEvent<T>() where T : EventData
        {
            EventData evt = EventData.Get(event_id);
            return evt as T;
        }
        ...
        {
            return state != WorldState.None && state != WorldState.Setup;
        }

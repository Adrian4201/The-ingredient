...
namespace RogueEngine
{
    ...
    public class Battle
    {
        ...
        public World world_data; //Access to world, not serialized to avoid loop reference
        ...
        {
            BattleCharacter character = GetActiveCharacter();
            return character != null && CanControlCharacter(player_id, character) && IsCharacterTurn(character);
        }

        public virtual bool IsPlayerActionTurn(int player_id)
        {
            BattleCharacter character = GetActiveCharacter();
            return character != null && CanControlCharacter(player_id, character) && IsCharacterActionTurn(character);
        }

        public virtual bool IsPlayerSelectorTurn(int player_id)
        {
            BattleCharacter character = GetActiveCharacter();
            return character != null && CanControlCharacter(player_id, character) && IsCharacterSelectorTurn(character);
        }
        ...
        public bool CanControlCharacter(int player_id, BattleCharacter character)
        {
            if (character == null)
                return false;
            Player cplayer = world_data.GetPlayer(character.player_id);
            if (cplayer != null && !cplayer.connected && cplayer.IsAlly() && player_id >= 0)
                return true;
            return character.player_id == player_id;
        }

        public bool CanControlCard(int player_id, Card card)
        {
            if (card == null)
                return false;
            Player cplayer = world_data.GetPlayer(card.player_id);
            if (cplayer != null && !cplayer.connected && cplayer.IsAlly() && player_id >= 0)
                return true;
            return card.player_id == player_id;
        }
        ...
        public Slot GetEmptySlot(bool enemy)
        {
            foreach (Slot slot in Slot.GetAll())
            {
                if (slot.enemy == enemy)
                {
                    BattleCharacter bslot = GetSlotCharacter(slot);
                    if (bslot == null)
                        return slot;
                }
            }
        ...
        public static void Clone(Battle source, Battle dest)
        {
            ...
            dest.world_data = source.world_data; //Just copy reference, not deep copy

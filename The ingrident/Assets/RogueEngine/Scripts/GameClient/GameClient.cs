...
namespace RogueEngine.Client
{
    ...
    public class GameClient : MonoBehaviour
    {
        ...
        public UnityAction<Champion, MapLocation> onMapMove;
        public UnityAction<Champion, EventData> onEventStart;
        public UnityAction<Champion, EventData> onEventChoice;
        public UnityAction<Champion, CardData> onRewardChoice;
        public UnityAction<Champion, CardData> onUpgradeCard;
        public UnityAction<Champion, CardData> onBuyCard;
        public UnityAction<Champion, CardData> onBuyItem;
        ...
        protected virtual void Start()
        {
            ...
            RegisterRefresh(GameAction.PlayerConnected, OnPlayerConnect);
            RegisterRefresh(GameAction.PlayerDisconnected, OnPlayerDisconnect);
            RegisterRefresh(GameAction.GameStarted, OnGameStart);
            RegisterRefresh(GameAction.GameEnded, OnGameEnd);
            RegisterRefresh(GameAction.GameTest, OnGameTest);

            RegisterRefresh(GameAction.MapMoved, OnMapMoved);
            RegisterRefresh(GameAction.MapEventStarted, OnMapEventStart);
            ...
            RegisterRefresh(GameAction.MapCardUpgraded, OnUpgradeCard);
            RegisterRefresh(GameAction.MapCardBought, OnBuyCard);
            RegisterRefresh(GameAction.MapItemBought, OnBuyItem);
        ...
        public void ChangeState(WorldState state)
        {
            ...
            msg.value = (int)state;
            SendAction(GameAction.ChangeState, msg);
        }

        //------ Map Actions

        public void MapMove(Champion champion, MapLocation location)
        {
        ...
        protected virtual void OnConnectedToGame(SerializedData sdata)
        {
            ...
            if(world_data.battle != null)
                world_data.battle.world_data = world_data; //Copy reference on unserialized world
        ...
        protected virtual void OnPlayerConnect(SerializedData sdata)
        {
            MsgInt msg = sdata.Get<MsgInt>();
            int player_id = msg.value;

            if (onPlayerConnect != null)
                onPlayerConnect.Invoke(player_id);

            if (connect_settings.IsOnline())
            {
                Player player = world_data.GetPlayer(player_id);
                if (player != null && onChatMsg != null)
                    onChatMsg.Invoke("", "Player Connected: " + player.username);
            }
        }

        protected virtual void OnPlayerDisconnect(SerializedData sdata)
        {
            MsgInt msg = sdata.Get<MsgInt>();
            int player_id = msg.value;

            if (onPlayerDisconnect != null)
                onPlayerDisconnect.Invoke(player_id);

            if (connect_settings.IsOnline())
            {
                Player player = world_data.GetPlayer(player_id);
                if (player != null && onChatMsg != null)
                    onChatMsg.Invoke("", "Player Disconnected: " + player.username);
            }
        }
        ...
        private void OnGameTest(SerializedData sdata)
        {
            onGameTest?.Invoke();
        }
        ...
        private void OnMapMoved(SerializedData sdata)
        {
            ...
            onMapMove?.Invoke(champion, loc);
        }

        private void OnMapEventStart(SerializedData sdata)
        {
            MsgMapEvent msg = sdata.Get<MsgMapEvent>();
            Champion champ = world_data.GetChampion(msg.champion_uid);
            EventData evt = EventData.Get(msg.event_id);
            onEventStart?.Invoke(champ, evt);
        }
        ...
        private void OnUpgradeCard(SerializedData sdata)
        {
            MsgChampionCard msg = sdata.Get<MsgChampionCard>();
            Champion champ = world_data.GetChampion(msg.champion_uid);
            CardData card = CardData.Get(msg.card_uid);
            onUpgradeCard?.Invoke(champ, card);
        }

        private void OnBuyCard(SerializedData sdata)
        {
            MsgChampionCard msg = sdata.Get<MsgChampionCard>();
            Champion champ = world_data.GetChampion(msg.champion_uid);
            CardData card = CardData.Get(msg.card_uid);
        ...
        private void OnBuyItem(SerializedData sdata)
        {
            MsgChampionCard msg = sdata.Get<MsgChampionCard>();
            Champion champ = world_data.GetChampion(msg.champion_uid);
            CardData card = CardData.Get(msg.card_uid);
            onBuyItem?.Invoke(champ, card);
        }
        ...
        {
            World wdata = GetWorld();
            return wdata.GetFirstChampion(GetPlayerID());
        }

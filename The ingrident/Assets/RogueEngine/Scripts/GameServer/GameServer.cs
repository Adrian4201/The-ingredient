...
namespace RogueEngine.Server
{
    ...
    public class GameServer
    {
        ...
        protected virtual void Init(World world, bool online)
        {
            ...
            RegisterAction(GameAction.ChangeState, ReceiveChangeState);
            ...
            world_logic.onGameTest += OnGameTest;

            world_logic.onMoved += OnMove;
            world_logic.onEventStarted += OnEventStarted;
            world_logic.onEventChoice += OnEventChoice;
            world_logic.onRewardChoice += OnRewardCardChoice;
            world_logic.onUpgradeCard += OnUpgradeCard;
            world_logic.onBuyCard += OnBuyCard;
            world_logic.onBuyItem += OnBuyItem;
        ...
        public void ReceiveMapMove(ClientData iclient, SerializedData sdata)
        {
            ...
            if (player != null && msg != null)
            {
                Champion champion = world_data.GetFirstChampion(player.player_id);
        ...
        public void ReceiveChangeState(ClientData iclient, SerializedData sdata)
        {
            MsgInt msg = sdata.Get<MsgInt>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null)
            {
                WorldState state = (WorldState)msg.value;
                world_logic.ChangeState(state);
            }
        }
        ...
        public void ReceiveMapRewardCard(ClientData iclient, SerializedData sdata)
        {
            ...
            {
                Champion champion = world_data.GetChampion(msg.champion_uid);
                if (champion != null && world_data.CanControlChampion(player.player_id, champion))
        ...
        public void ReceiveMapUpgrade(ClientData iclient, SerializedData sdata)
        {
            ...
            if (player != null)
            {
                ...
                if (champion != null && card != null && world_data.CanControlChampion(player.player_id, champion))
        ...
        public void ReceiveBuyItem(ClientData iclient, SerializedData sdata)
        {
            ...
            if (player != null)
            {
                ...
                if (champion != null && item != null && world_data.CanControlChampion(player.player_id, champion))
            ...
            if (player != null)
            {
                if (world_data.state == WorldState.Battle)
                {
                    if (world_data.battle != null && world_data.battle.IsPlayerActionTurn(player.player_id) && !battle_logic.IsResolving())
                    {
                        ...
                        if (character != null && item != null && world_data.battle.CanControlCharacter(player.player_id, character))
        ...
        public void ReceivePlayCard(ClientData iclient, SerializedData sdata)
        {
            ...
            if (player != null && msg != null && world_data.battle != null && world_data.battle.IsPlayerActionTurn(player.player_id) && !battle_logic.IsResolving())
            {
                ...
                if (card != null && world_data.battle.CanControlCard(player.player_id, card))
        ...
        public void ReceiveMoveCharacter(ClientData iclient, SerializedData sdata)
        {
            ...
            {
                BattleCharacter character = world_data.battle.GetCharacter(msg.character_uid);
                if (character != null && world_data.battle.CanControlCharacter(player.player_id, character))
        ...
        public void ReceiveCastCardAbility(ClientData iclient, SerializedData sdata)
        {
            ...
            if (player != null && msg != null && world_data.battle != null && world_data.battle.IsPlayerActionTurn(player.player_id) && !battle_logic.IsResolving())
            {
                ...
                //if (card != null && world_data.CanControlCard(player.player_id, card))
        ...
        public void ReceiveSelectCost(ClientData iclient, SerializedData sdata)
        {
            ...
            if (player != null && msg != null && world_data.battle != null && world_data.battle.IsPlayerSelectorTurn(player.player_id) && !battle_logic.IsResolving())
            {
                battle_logic.SelectCost(msg.value);
            }
        }
        ...
        public void RemoveClient(ClientData client)
        {
            ...
            if (player != null && player.connected)
            {
                ...
                msg.value = player.player_id;
                SendToAll(GameAction.PlayerDisconnected, msg, NetworkDelivery.Reliable);

                //battle_logic.RemovePlayerCharacters(player);
            }
        }
        ...
        public void AfterPlayerConnected(int player_id)
        {
            MsgInt msg = new MsgInt();
            msg.value = player_id;
            SendToAll(GameAction.PlayerConnected, msg, NetworkDelivery.Reliable);
        }
        ...
        protected virtual void OnGameTest()
        {
            SendToAll(GameAction.GameTest);
        }
        ...
        protected virtual void OnEventStarted(Champion champ, EventData evt)
        {
            MsgMapEvent msg = new MsgMapEvent();
            msg.champion_uid = champ.uid;
            msg.event_id = evt.id;
            SendToAll(GameAction.MapEventStarted, msg, NetworkDelivery.Reliable);
        }
        ...
        protected virtual void OnUpgradeCard(Champion champ, CardData card)
        {
            MsgChampionCard msg = new MsgChampionCard();
            msg.champion_uid = champ.uid;
            msg.card_uid = card.id;
            SendToAll(GameAction.MapCardUpgraded, msg, NetworkDelivery.Reliable);
        }

        protected virtual void OnBuyCard(Champion champ, CardData card)
        {
            MsgChampionCard msg = new MsgChampionCard();
            msg.champion_uid = champ.uid;
            msg.card_uid = card.id;
            SendToAll(GameAction.MapCardBought, msg, NetworkDelivery.Reliable);
        }

        protected virtual void OnBuyItem(Champion champ, CardData card)
        {
            MsgChampionCard msg = new MsgChampionCard();
            msg.champion_uid = champ.uid;
            msg.card_uid = card.id;
            SendToAll(GameAction.MapItemBought, msg, NetworkDelivery.Reliable);
        }
